...
- 채널: 정기 보고=이메일(본인 발송, `soobin7612@gmail.com`) / **실패·긴급 경보=ntfy**(Gmail과 분리, 스마트폰 Urgent 푸시)
- 호스팅: Synology NAS(Docker, 상시) 권장
- 확정 사양(SRS)을 임의로 바꾸지 않는다. 변경 필요 시 SRS 버전업 후 반영.

## 백로그 (v1.3 재정렬 — '개인 전담 비서' / 신뢰성 우선)

기반 공사 후 신뢰성(연락두절·긴급누락 방지) → 편의 순.

0. **기반**: n8n 지속성 확보(세션 비의존 상시 구동) + L-7 수정본 Execute 확인
1. **독립 알림 + 패스트트랙** (최우선 · 신뢰성 키스톤)
   - 실패 알림을 **Gmail → ntfy**로 교체(NFR-8 채널 독립성; 기존 Gmail 에러핸들러 결함 수정)
   - **실시간 패스트트랙**(FR-10): 엄격 화이트리스트(발신자 AND 긴급키워드) → ntfy Urgent 경보
2. **모바일 보고**: NAS 정적 호스팅 + 비밀 링크(첨부 대신 URL) — 폰 열람 (구 #4)
3. **지능 고도화**: 분류를 Gemini 노드로 (문맥 판단, L-1 해소) — 비용·키 필요, 마지막 (구 #2)
4. 운영 안정화(중복 발신자 압축 등), 0건 노드 스킵(AC-3.2) 검증
5. **리팩토링 (저우선, 여유 시)**: 대시보드 방어심층(render 출력단 추가 이스케이프/`textContent`) + 검색 debounce. **실제 취약점·성능 문제 아님**(XSS는 상류 esc()로 이미 차단, 데이터는 48h 창 ~수십 건). 외부 소스평가 대비 '아머' 겸 부드러움 개선용.
